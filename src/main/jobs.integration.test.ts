...
import { FFMPEG_PATH, FFPROBE_PATH } from './ffmpeg/binaries'
...
describe('startExportJob (integração — exige ffmpeg instalado)', () => {
  it('gera uma parte por segmento, com as durações certas', async () => {
    ...
    const job = startExportJob(FFMPEG_PATH, request({ outputDir: dirPartes }), () => {})
  ...
  it('nomeia as partes em ordem', async () => {
    ...
    const job = startExportJob(FFMPEG_PATH, request({ outputDir: dirNomes }), () => {})
    await job.promise
  ...
  it('nunca sobrescreve arquivo existente', async () => {
    ...
    const job1 = startExportJob(FFMPEG_PATH, request({ outputDir: collisionDir }), () => {})
    await job1.promise
    const job2 = startExportJob(FFMPEG_PATH, request({ outputDir: collisionDir }), () => {})
  ...
  it('reporta progresso geral crescente de 0 a 1', async () => {
    ...
    const job = startExportJob(FFMPEG_PATH, request({ outputDir: dirProgresso }), (p) =>
      seen.push(p)
    )
    await job.promise
  ...
  it('para a fila e devolve erro quando o ffmpeg falha', async () => {
    const job = startExportJob(
      FFMPEG_PATH,
      request({ inputPath: join(dir, 'fantasma.mp4') }),
      () => {}
    )
    const result = await job.promise
